...
public interface AccTransactionDao {
    ...
    List<AccTransaction> getTransactionsByType(int accType);

    @Query("SELECT * FROM transactions WHERE category_id = :category_id")
    List<AccTransaction> getTransactionsByCateogry(int category_id);

    @Query("SELECT * FROM transactions WHERE amount BETWEEN :minAmount AND :maxAmount")
    List<AccTransaction> getTransactionsBetweenAmount(double minAmount, double maxAmount);

    @Query("SELECT * FROM transactions WHERE transaction_time BETWEEN :minDate AND :maxDate")
    List<AccTransaction> getTransactionsBetweenTime(int minDate, int maxDate);
